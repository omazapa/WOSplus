...
def df_split(
    ...
    * on_not_condition or on_not_conditions are deprecated usage
    ...
    if on_contains is not None:
        df_pureLEFT = df[df[on].str.contains(
            on_contains)].reset_index(drop=True)
        df_pureRIGHT = df[~df[on].str.contains(
            on_contains)].reset_index(drop=True)
    elif Operator is not None and condition is not None:
        df_pureLEFT = df[otrue[Operator](
            df[on], condition)].reset_index(drop=True)
        df_pureRIGHT = df[ofalse[Operator](
            df[on], condition)].reset_index(drop=True)
    else:
...
def merge_by_series(
    ...
    new_right = kk[np.logical_and(kk[left_on] == '', kk[right_on] != '')].drop(
        left_keys, axis='columns')
    if left_series.shape[0] or right_series.shape[0]:
